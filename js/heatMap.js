...
function setMap() {
  //document.getElementById('test') = "here";
 // map width and height, matches 
...
function loadData() {
   queue()   // queue function loads all external data files asynchronously 
    .defer(d3.json, "world-110m.json")  // our geometries
...
function processData(error,world,countryData) {
  ...
  for (var i in countries) {    // for each geometry object
    for (var j in countryData) {  // for each row in the CSV
      var ceyan = countries[i];
      if(ceyan.id == countryData[j].id) {   // if they match
        for(var k in countryData[i]) {   // for each column in the a row within the CSV
          if(k != 'name' && k != 'id') {  // let's not add the name or id as props since we already have them
            ...
            ceyan.properties[k] = Number(countryData[j][k])  // add each CSV column key/value to geometry object
          } 
        }
  ...
  d3.select('#clock').html(countryData.id);  // populate the clock initially with the current year
  drawMap(world);  // let's mug the map now with our newly populated data object
}

function drawMap(world) {
    var guojia = topojson.feature(world, world.objects.countries).features
    svg.selectAll(".country")   // select country objects (which don't exist yet)
      .data(guojia)  // bind data to these non-existent objects
      .enter().append("path",".graticule") // prepare data to be appended to paths
...
function animateMap() {
  ...
  init.width = function(_) {
    if (!arguments.length) return width;
    width = _;
    return init();
  };

  init.height = function(_) {
    if (!arguments.length) return height;
    height = _;
    return init();
  };

 return init; // magic starts here
}
